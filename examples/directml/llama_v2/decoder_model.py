...
class DecoderModel(torch.nn.Module):
    ...
    def forward_no_cache(self, x, attn_mask, cache):
        ...
        unpadded_attn_mask = torch.ones([attn_mask.shape[0], 1, attn_mask.shape[2] - next_seq_len], dtype=torch.int32)
...
class ApplyMask(torch.nn.Module):
    ...
    def forward(self, score, attn_mask, pos, pos_end):
        score = score + attn_mask[:, pos:pos_end, :pos_end] * -10000.0
        return score
...
class SelfAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if use_cache:
            ...
            k_cache = k_cache[:, :, pos_end:, :]
            v_cache = v_cache[:, :, pos_end:, :]
