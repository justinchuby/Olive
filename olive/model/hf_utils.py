...
def load_huggingface_model_from_model_class(model_class: str, name: str, device: Device = Device.CPU, **kwargs):
    """
    Load huggingface model from model_loader and name
    """
    return (
        huggingface_model_loader(model_class)(name, **kwargs).to("cuda")
        if device == Device.GPU
        else huggingface_model_loader(model_class)(name, **kwargs)
    )
